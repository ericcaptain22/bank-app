...
void show_hint(const char *hint) {
    gtk_label_set_text(GTK_LABEL(message_label), hint);
}
...
void on_login_button_clicked(GtkWidget *widget, gpointer data) {
    ...
    show_hint("Invalid credentials! Try again.");
}
...
void on_create_account_confirm_button_clicked(GtkWidget *widget, gpointer data) {
    ...
    if (strlen(username) == 0 || strlen(password) == 0) {
        ...
        show_hint("Hint: Ensure all fields are filled correctly.");
        return;
    }

    if (initial_deposit < 0) {
        g_print("Error: Initial deposit must be non-negative.\n");
        show_hint("Hint: Check the initial deposit validation.");
        return;
    }

    create_account(username, password, initial_deposit);
    g_print("Account created successfully for %s.\n", username);
    show_hint("Account created. Return to the login page.");
}
...
void on_user_action_clicked(GtkWidget *widget, gpointer data) {
    ...
    show_hint("Hint: Check the implementation of this function.");
}

void on_admin_action_clicked(GtkWidget *widget, gpointer data) {
    const char *action = (const char *)data;
    g_print("Admin action: %s\n", action);
    show_hint("Hint: Check admin-specific functionality.");
}
...
void on_deposit(GtkWidget *widget, gpointer data) {
    ...
    if (amount <= 0) {
        gtk_label_set_text(GTK_LABEL(user_message_label), "Error: Deposit amount must be greater than 0.");
        return;
    }
...
void on_withdraw(GtkWidget *widget, gpointer data) {
    ...
    if (amount <= 0 || amount > users[current_user_index].balance) {
        gtk_label_set_text(GTK_LABEL(user_message_label), "Error: Invalid withdrawal amount.");
        return;
    }
...
void on_transfer(GtkWidget *widget, gpointer data) {
    ...
    if (amount <= 0 || amount > users[current_user_index].balance) {
        gtk_label_set_text(GTK_LABEL(user_message_label), "Error: Invalid transfer amount.");
        return;
    }
    ...
    gtk_label_set_text(GTK_LABEL(user_message_label), "Error: Recipient not found.");
}
